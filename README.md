# DevOps Lab Documentation

![Docs](https://img.shields.io/badge/Docs-Kubernetes%20%7C%20Docker%20%7C%20VMware-blue)
![Environment](https://img.shields.io/badge/Environment-Local%20%7C%20AWS-green)
![Tools](https://img.shields.io/badge/Tools-Minikube%20%7C%20Swarm%20%7C%20vmrun-orange)
![License](https://img.shields.io/badge/License-MIT-lightgrey)

This repository contains a collection of hands-on guides for setting up and managing modern infrastructure and container orchestration environments.

It covers:

- Kubernetes (Multi-node cluster using kubeadm)
- Minikube (Single-node local cluster)
- Docker Swarm (Cluster orchestration)
- VMware Fusion (VM automation using CLI)

---

## Documentation Index

### Kubernetes Multi-Node Cluster

Setup a production-style Kubernetes cluster using kubeadm, containerd, and Flannel.

- **Guide:** [Kubernetes.md](./Kubernetes.md)

---

### Minikube (Single Node Cluster)

Run a local Kubernetes cluster for learning and development.

- **Guide:** [Minikube.md](./Minikube.md)

---

### Docker Swarm Cluster

Learn Docker’s native orchestration with multi-node cluster setup and service management.

- **Guide:** [Swarm.md](./Swarm.md)

---

### VMware Fusion Automation (vmrun)

Manage virtual machines using CLI without navigating directories.

- **Guide:** [Vmware.md](./Vmware.md)

---

## Purpose

This repository is intended for:

- Learning container orchestration (Kubernetes & Swarm)
- Practicing real-world cluster setups
- Local development environments
- Infrastructure automation

## Contribution

Contributions are welcome—feel free to improve documentation, add guides, or fix issues.

## License

See the [LICENSE](./LICENSE) file for details.
